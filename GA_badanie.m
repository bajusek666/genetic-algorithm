...
sigma = 0.01 % Odchylnie standardowe rozdkłądu Normalnego
...
% Początkowa macierz binarna, jednakowa populacja początkowa dla każdej
% iteracji algorytmu
rng_seed = rng;
rng(100);
P0 = randi([0,1], m, l);
...
hold on;
    ...
    % Inicjalizacja macierzy na wyniki z 15 uruchomień
    % Każdy wiersz to jedno uruchomienie, każda kolumna to generacja
    all_max_runs = zeros(num_runs, lg + 1);
    all_mean_runs = zeros(num_runs, lg + 1);
    ...
    for run = 1:num_runs
        ...
        for i = 0:lg
            ...
            current_max = [current_max, max(fitness)];
            current_mean = [current_mean, mean(fitness)];
    ...
    % Uśrednianie wyników po 15 iteracjach
    avg_max_fit = mean(all_max_runs, 1);
    avg_mean_fit = mean(all_mean_runs, 1);

    % Wykreślanie wyników dla danego m
    x = 0:lg;
    plot(x, avg_max_fit, 'LineWidth', 2);
    plot(x, avg_mean_fit, 'LineWidth', 1);

title("Wyłączone krzyżowanie");
xlabel("Generacja");
ylabel("Fitness");
